...
import {
  ...
  Text,
  ScrollView,
  TouchableNativeFeedback, TextInput, StatusBar,
  Linking,
  Alert
} from 'react-native'
...
class Chat extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.route.params?.data) {
      const { chats, socket, receiverId, customer, orderId, status } = this.props.route.params?.data
      this.setState({
        chats,
        socket,
        customer,
  ...
  componentWillUnmount() {
    ...
    if (this.props.route.params?.statusbar) {
      ...
      StatusBar.setBackgroundColor(this.props.route.params?.statusbar.background, true)
    }
    this.setState({
      socket: null
    })
  ...
  _onSendChat = () => {
    const { socket, chatText, customer, orderId } = this.state
          ...
          <View style={{ padding: 10, paddingVertical: 15, paddingLeft: 15 }}>
            ...
            >
              <View style={{ width: 35, height: 35, borderRadius: 35 / 2, alignItems: 'center', justifyContent: 'center', backgroundColor: Color.primary, overflow: 'hidden' }}>
                <Fa iconStyle='solid' color={Color.black} size={20} name="chevron-left" />
              </View>
            </TouchableNativeFeedback>
          </View>
            ...
            <View style={{ paddingRight: 10 }}>
              {
                customer !== null ?
                  <View>
                    <Text numberOfLines={1} style={{ fontSize: 17, marginBottom: 3, fontWeight: 'bold' }}>{customer.userName}</Text>
                  </View>
                  :
                  <View>
            ...
            <View style={{ marginHorizontal: 2.5 }}>
              ...
              >
                <View style={{ width: 40, height: 40, borderRadius: 40 / 2, alignItems: 'center', justifyContent: 'center', backgroundColor: Color.secondary, overflow: 'hidden' }}>
                  <Fa iconStyle='solid' color={colorYiq(Color.secondary)} size={18} name="phone" />
                </View>
              </TouchableNativeFeedback>
            </View>
            <View style={{ marginHorizontal: 2.5 }}>
              <TouchableNativeFeedback
                onPress={() => Linking.openURL(`whatsapp://send?phone=${phoneNumFormat(customer.userPhone)}`)}
                useForeground={true}
                background={TouchableNativeFeedback.Ripple('rgba(0,0,0,.25)', false)}
              >
                <View style={{ width: 40, height: 40, borderRadius: 40 / 2, alignItems: 'center', justifyContent: 'center', backgroundColor: Color.secondary, overflow: 'hidden' }}>
        ...
        <View style={{ paddingHorizontal: 15, paddingVertical: 10, flexDirection: 'row', alignItems: 'flex-end', backgroundColor: Color.grayLighter }}>
          {
            this.state.status === 'completed' || this.state.status === 'cancelled_by_driver' || this.state.status === 'cancelled_by_user' ?
              ...
              <TextInput
                ...
                style={{ flex: 1, color: Color.textColor, fontFamily: 'Archivo', fontSize: 14, backgroundColor: Color.white, paddingVertical: 6, paddingHorizontal: 15, minHeight: 45, maxHeight: 90, borderRadius: 45 / 2, elevation: 1 }} placeholder='Masukkan pesan'
              />
          }
          {
            this.state.status === 'completed' || this.state.status === 'cancelled_by_driver' || this.state.status === 'cancelled_by_user' ?
              <View style={{ width: 45, height: 45, marginLeft: 10, borderRadius: 45 / 2, backgroundColor: Color.gray, alignItems: 'center', justifyContent: 'center', elevation: 1, overflow: 'hidden' }}>
                <Fa iconStyle='solid' size={18} color={colorYiq(Color.gray)} name='paper-plane' />
              </View>
              :
              <TouchableNativeFeedback
              ...
              >
                <View style={{ width: 45, height: 45, marginLeft: 10, borderRadius: 45 / 2, backgroundColor: Color.green, alignItems: 'center', justifyContent: 'center', elevation: 1, overflow: 'hidden' }}>
                  <Fa iconStyle='solid' size={18} color={colorYiq(Color.green)} name='paper-plane' />
                </View>
              </TouchableNativeFeedback>
          }
        </View>
