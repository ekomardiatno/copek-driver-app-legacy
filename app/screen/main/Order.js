        ...
        <Header title='Pesanan' />
        {
          orders.length <= 0 ?
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 30 }}>
              <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 8 }}>Semangat!</Text>
              <Text style={{ textAlign: 'center', lineHeight: 18, color: Color.textMuted }}>Setiap pesanan masuk yang anda kerjakan akan tercatat disini.</Text>
            </View>
            :
            <ScrollView>
              {
                orders.map((order, i) => (
                  order.orderType === 'FOOD' ?
                    <OrderHistoryItem last={i === orders.length - 1 ? true : false} onPress={() => this._navigateBooking(order)} key={order.orderId} origin={order.origin} destination={order.destination} dateTime={dateFormatted(order.date, true)} type={order.orderType} fare={Currency(order.carts.map(function (a) {
                      return a.foodPrice * a.qty
                    }).reduce(function (a, b) {
                      return a + b
                    }) + order.fare)} status={order.status} />
                    :
                    <OrderHistoryItem last={i === orders.length - 1 ? true : false} onPress={() => this._navigateBooking(order)} key={order.orderId} origin={order.origin} destination={order.destination} dateTime={dateFormatted(order.date, true)} type={order.orderType} fare={Currency(order.fare)} status={order.status} />
                ))
              }
            </ScrollView>
        }
